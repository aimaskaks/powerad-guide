  ...
  "recommendedParties": [
    {
      ...
      "reason": "得意訓練のシナジーとスペシャルタッグ効率を中心に150字程度で説明。スキル名は一切記載しないこと。理想のキャラが所持できていない場合は『本来は〇〇が理想だが、所持キャラの〇〇で代用』のように代用情報も記載すること"
    }
  ],
  "wantedCharacters": [
...
厳守事項：
- membersには必ず「ユーザーの所持キャラ」に記載されたキャラのみを使用すること。所持していないキャラを編成に入れることは絶対禁止
- 所持キャラで理想の編成が組めない場合は、得意訓練やスキルが合わなくても所持キャラで6枠を埋めること
- recommendedPartiesは剣士・弓使い・魔法使い・魔闘士から所持キャラで作れる最大3パーティ
- 同じジョブでも属性が違えば必殺技・ASを教えられるキャラが異なるため、別パーティとして提案してよい
- 例：剣士×火（マモルが必殺技担当）と剣士×水（カルメノが必殺技担当）は別パーティとして成立する
- パーティの多様性は「ジョブの違い」ではなく「必殺技・AS担当キャラの違い」を基準にすること
- titleは必ず「【ジョブ名×属性名】」の形式（例：【剣士×火属性】【弓使い×水属性】）
- titleの属性名は「火属性」「水属性」「風属性」のいずれか1つのみ。「混合」「複数」は絶対に使用しない
- 「得意訓練の組み合わせ（2:2や3人固め）」を最優先で考える
- 各パーティはマリセアを所持していれば必ず含める
- メンバーは必ず6人（マリセア含む）
- 得意訓練は必ず3人固める or 2:2で固める
- 冒険者が習得できる必殺技は1つ・アクションスキルは1つのみ
- 必殺技を教えられるキャラは1人だけ選ぶ（得意訓練目的など別の理由で同じキャラが入る場合は除く）
- ASを教えられるキャラは1人だけ選ぶ（得意訓練目的など別の理由で同じキャラが入る場合は除く）
- 残り枠はサポートスキル担当で埋める
